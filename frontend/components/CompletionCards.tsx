...
const CompletionCards = async () => {
  ...
  if (!user || !user.is_superuser)
    return <p className="text-red-500 text-6xl font-bold font-figtree">Unauthorized</p>;

  const cookieStore = await cookies();
  const token = cookieStore.get('access_token')?.value || '';

  const getPendingCompletions = async (): Promise<PendingCompletion[]> => {
    const res = await fetch(`${API_BASE_URL}/api/v1/completions/pending`, {
  ...
  return (
    <>
      <div className="mb-8 space-y-15">
        ...
        <h2 className="text-4xl font-bold font-figtree text-neutral-400 text-shadow-xs">
