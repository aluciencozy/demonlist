...
def get_completion(
    ...
    stmt = select(Completion).where(
        Completion.demon_id == demon_id, Completion.user_id == current_user.id
    )
...
def submit_completion(
    ...
    stmt = select(Completion).where(
        Completion.demon_id == completion.demon_id,
    ...
    if existing_completion and existing_completion.status != Status.REJECTED:
        raise HTTPException(
            ...
            detail="Completion for this demon already exists.",
        )
...
def submit_completion_file(
    ...
    if file.content_type not in ["video/mp4", "video/quicktime", "video/webm"]:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Invalid file type. Only MP4, MOV, WEBM allowed.",
        )
    ...
    if file.size is not None and file.size > 100 * 1024 * 1024:
        raise HTTPException(
            status.HTTP_400_BAD_REQUEST, detail="File size exceeds the 100MB limit"
        )
    ...
    if not url:
        raise HTTPException(
            ...
            detail="Failed to upload completion video",
        )
...
def update_completion_status(
    db: SessionDep,
    completion_id: int,
    update_data: CompletionStatusUpdate,
    current_user: Annotated[User, Depends(get_current_admin_user)],
):
    if not current_user or not current_user.is_superuser:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Not authorized to update completion status.",
        )
    ...
    if update_data.status == Status.REJECTED:
        s3_service.delete_file(completion_to_update.proof_link)
